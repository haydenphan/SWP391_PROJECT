...
<html lang="en">
    <head>
        ...
        <link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/css/lecturecss.css">
    ...
    <body>
            ...
            <div class="info-section">
                ...
                <ul>
                    <c:forEach var="material" items="${lecture.lectureMaterials}">
                        <li><a href="${material.lectureMaterialUrl}">Download Material</a></li>
                        </c:forEach>
                </ul>
            </div>
        </div>
        ...
        <div class="navigation-buttons">
            <button id="prevLectureButton">Previous Lecture</button>
            <button id="nextLectureButton" style="display: none;">Next Lecture</button>
        </div>
        ...
        <script>
            // Making sectionID available in JavaScript
            const sectionID = '${sectionID}';

            document.addEventListener('DOMContentLoaded', function () {
                const video = document.getElementById('lectureVideo');
                const nextLectureButton = document.getElementById('nextLectureButton');
                const lectureId = '${lecture.lectureID}';
                const progressUrl = '${pageContext.request.contextPath}/getProgress?lectureId=' + lectureId;

                // Fetch the saved progress and set the video time
                fetch(progressUrl)
                        .then(response => response.json())
                        .then(data => {
                            if (data && data.watchedPercentage) {
                                const savedTime = (data.watchedPercentage / 100) * video.duration;
                ...
                video.addEventListener('timeupdate', function () {
                    ...
                    if (percentageWatched >= 80) {
                        nextLectureButton.style.display = 'inline-block';
                    }
                    // Send progress to server every 5 seconds
                    if (Math.floor(video.currentTime) % 5 === 0) {
                        sendProgressToServer(percentageWatched);
                    }
                });

                function sendProgressToServer(percentageWatched) {
                    fetch('${pageContext.request.contextPath}/trackProgress', {
                        method: 'POST',
                        headers: {
                        ...
                        body: JSON.stringify({
                            lectureID: lectureId,
                            watchedPercentage: percentageWatched
                        })
                    }).then(response => response.json())
                ...
                // Other functions for replies, edits, etc.
            });
        </script>
        <script>
