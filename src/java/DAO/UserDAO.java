...
public class UserDAO extends DAO<User> {
    ...
    public List<User> getTopInstructors() throws Exception {
        List<User> instructors = new ArrayList<>();
        String sql = "SELECT TOP 5 u.UserID, u.Username, u.FirstName, u.LastName, u.Email, "
                + "SUM(c.TotalEnrolled) AS TotalLearners "
                + "FROM Users u "
        ...
        try (Connection con = JDBC.getConnectionWithSqlJdbc(); PreparedStatement st = con.prepareStatement(sql)) {
            try (ResultSet rs = st.executeQuery()) {
                while (rs.next()) {
                    ...
                    instructor.setTotalLearners(rs.getInt("TotalLearners"));
...
//    }
    public static void main(String[] args) throws Exception {
        ...
        List<User> topInstructors = userDAO.getTopInstructors();

        for (User instructor : topInstructors) {
            System.out.println("UserID: " + instructor.getUserID()
                    + ", Username: " + instructor.getUserName()
                    + ", FirstName: " + instructor.getFirstName()
                    + ", LastName: " + instructor.getLastName()
                    + ", Email: " + instructor.getEmail()
                    + ", Total Learners: " + instructor.getTotalLearners());
        }
    }
}
